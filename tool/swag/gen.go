...
import (
	...
	enumGen "capstone/tool/swag/enum_gen"
...
var PostSwaggerDefinitions = make(map[string]spec.Schema, 0)

func init() {
	for definitionKey, customSchema := range enumGen.PostSwaggerDefinitions {
		PostSwaggerDefinitions[definitionKey] = customSchema
	}
}
...
func (g *Gen) Build(config *Config) error {
	...
	for definitionKey, customSchema := range PostSwaggerDefinitions {
		if _, ok := swagger.Definitions[definitionKey]; ok {
			swagger.Definitions[definitionKey] = customSchema
		}
	}
